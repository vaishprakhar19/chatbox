...
function Navbar() {
    const { setIsLoggedIn, activeUsers, setShowActiveUsers, isGroupChat } = useAppState();
    ...
    return (
        <div className='navbar'>
            <div className="logo">
                <img className='profile-img' src={auth.currentUser.photoURL} alt="" onError={handleImageError} />
            ...
            <div className="nav-buttons">
                ...
                {isGroupChat && <button className='button' onClick={handleToggleActiveUsers}>Active: {activeUsers.length}</button>}
            </div>
        </div>
    );
}
